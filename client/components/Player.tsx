...
const Player = () => {
  const { pause, volume, active, currentTime, collapsed } =
    useTypedSelector((state) => state.player)
  const { tracks } = useTypedSelector(
    (state) => state.track
  )
  const {
    pauseTrack,
  ...
  const setAudio = () => {
    if (active) {
      ...
      audio.onended = () => {
        ...
        nextTrackIndex = tracks[nextTrackIndex]
          ? nextTrackIndex
          : 0
  ...
  return (
    <div
      className={clsx({
        [styles.player]: true,
        [styles.player_collapsed]: collapsed,
      })}>
      <Button
        className={styles['collapse-btn']}
        variant='contained'
        onClick={() => setCollapsed(!collapsed)}
        style={{ background: '#e0d100', color: '#000000' }}>
        {collapsed ? <RevealIcon /> : <CollapseIcon />}
      </Button>
